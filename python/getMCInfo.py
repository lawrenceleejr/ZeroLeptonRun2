...
""" % config.outputfilename.split('.')[0])
        ...
        import pickle
        pointdict = {}
        if os.path.isfile('signalPointPickle.pkl'):
...
""")
        for key,info in sorted(myMap.items()):
            ...
            massstr = ''
            for d in pointdict.itervalues():
                if d.has_key(channel):
                    massstr = str(d[channel])
            ftex.write("%d & %s & %.3g & %d & %.5g \\\\\n" % (channel,massstr,xsecDB.rawxsect(channel,hardproc) * xsecDB.efficiency(channel,hardproc),info[0],lint))
        ftex.write(r"""\hline
\end{tabular}
\end{center}
...
def parseCmdLine(args):
    ...
    parser.add_option("--no", dest="NB", help="position of mc_channel_id in the string (splitted by the character '.')", default=4)
