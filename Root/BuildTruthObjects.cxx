...
bool BuildTruthObjects::processEvent(xAOD::TEvent& event)
{
  ...
  //std::pair< xAOD::TruthParticleContainer*, xAOD::AuxContainerBase* > truthmuonsIso ;
  xAOD::TruthParticleContainer* truthmuonsIso = new xAOD::TruthParticleContainer() ;
  xAOD::AuxContainerBase* truthmuonsIsoAux    = new xAOD::AuxContainerBase();
  truthmuonsIso->setStore( truthmuonsIsoAux );

  for(xAOD::TruthParticleContainer::const_iterator mu_itr = truthmuons->begin();
      mu_itr != truthmuons->end(); ++mu_itr ){
    if(!((*mu_itr)->auxdata<unsigned int>("classifierParticleType")==6)) continue;
    xAOD::TruthParticle* mu = new xAOD::TruthParticle();
    truthmuonsIso->push_back( mu );
    *mu = **mu_itr;
  }
  if ( ! store->record(truthmuonsIso,"myTruthMuonsIso"+m_suffix).isSuccess() ) {
    throw std::runtime_error("Could not store myTruthMuonsIso"+m_suffix);
  }
  if ( ! store->record(truthmuonsIsoAux,"myTruthMuonsIso"+m_suffix+"Aux.").isSuccess()) {
    throw std::runtime_error("Could not store myTruthMuonsIso"+m_suffix+"Aux.");
  }
  ...
  //std::pair< xAOD::TruthParticleContainer*, xAOD::AuxContainerBase* > truthelectronsIso ;
  xAOD::TruthParticleContainer* truthelectronsIso = new xAOD::TruthParticleContainer() ;
  xAOD::AuxContainerBase* truthelectronsIsoAux    = new xAOD::AuxContainerBase();
  truthelectronsIso->setStore( truthelectronsIsoAux );

  for(xAOD::TruthParticleContainer::const_iterator el_itr = truthelectrons->begin();
      el_itr != truthelectrons->end(); ++el_itr ){
    if(!((*el_itr)->auxdata<unsigned int>("classifierParticleType")==2)) continue;
    xAOD::TruthParticle* el = new xAOD::TruthParticle();
    truthelectronsIso->push_back( el );
    *el = **el_itr;
  }

  if ( ! store->record(truthelectronsIso,"myTruthElectronsIso"+m_suffix).isSuccess() ) {
    throw std::runtime_error("Could not store myTruthElectronsIso"+m_suffix);
  }
  if ( ! store->record(truthelectronsIsoAux,"myTruthElectronsIso"+m_suffix+"Aux.").isSuccess()) {
    throw std::runtime_error("Could not store myTruthElectronsIso"+m_suffix+"Aux.");
  }
  ...
  // Isolated photons
  //std::pair< xAOD::TruthParticleContainer*, xAOD::AuxContainerBase* > truthphotonsIso ;
  xAOD::TruthParticleContainer* truthphotonsIso = new xAOD::TruthParticleContainer() ;
  xAOD::AuxContainerBase* truthphotonsIsoAux    = new xAOD::AuxContainerBase();
  truthphotonsIso->setStore( truthphotonsIsoAux );

  for(xAOD::TruthParticleContainer::const_iterator ph_itr = truthphotons->begin();
      ph_itr != truthphotons->end(); ++ph_itr ){
    if(!((*ph_itr)->auxdata<unsigned int>("classifierParticleType")==14)) continue;
    xAOD::TruthParticle* ph = new xAOD::TruthParticle();
    truthphotonsIso->push_back( ph );
    *ph = **ph_itr;
  }

  // TEST GERALDINE
  for(xAOD::TruthParticleContainer::const_iterator phiso_itr = truthphotonsIso->begin();
      phiso_itr != truthphotonsIso->end(); ++phiso_itr ){
    bool phiso_sel=0;
    if( (*phiso_itr)->pt() > 25000 && std::abs((*phiso_itr)->eta()) < 2.37 )
      phiso_sel = 1 ;
    if( !phiso_sel ){
      (*phiso_itr)->auxdecor<char>("passOR") = 0 ; 
      continue;
    }else{
      (*phiso_itr)->auxdecor<char>("passOR") = 1 ; 
    }
  }


  if ( ! store->record(truthphotonsIso,"myTruthPhotonsIso"+m_suffix).isSuccess() ) {
    throw std::runtime_error("Could not store myTruthPhotonsIso"+m_suffix);
  }
  if ( ! store->record(truthphotonsIsoAux,"myTruthPhotonsIso"+m_suffix+"Aux.").isSuccess()) {
    throw std::runtime_error("Could not store myTruthPhotonsIso"+m_suffix+"Aux.");
  }
  ...
  if ( m_PhotonInOR ) {
    if (! OverlapRemoval(truthelectronsIso, truthmuonsIso, outputjets, truthphotonsIso, false, 0.2, 0.4, 0.4, 0.01, 0.05, 0.2, 0.4, 0.000) ) throw std::runtime_error("Error in OverlapRemoval");
  }
  else{
  if (! OverlapRemoval(truthelectronsIso, truthmuonsIso, outputjets, false, 0.2, 0.4, 0.4, 0.01, 0.05) ) throw std::runtime_error("Error in OverlapRemoval");
  }
...
bool BuildTruthObjects::OverlapRemoval(const xAOD::TruthParticleContainer *electrons, const xAOD::TruthParticleContainer *muons, const xAOD::JetContainer *jets, bool doHarmonization, double dRejet, double dRjetmu, double dRjete, double dRemu, double dRee)
{
  ...
  for( ; mu_itr != mu_end; ++mu_itr ) {
    ...
    if( (*mu_itr)->pt() > 10000 && std::abs((*mu_itr)->eta()) < 2.7 )
      mu_sel = 1 ; 
    if(mu_sel){
...
bool BuildTruthObjects::OverlapRemoval(const xAOD::TruthParticleContainer *electrons, const xAOD::TruthParticleContainer *muons, const xAOD::JetContainer *jets, const xAOD::TruthParticleContainer *photons, const bool doHarmonization, const double dRejet, const double dRjetmu, const double dRjete, double dRemu, double dRee, double dRphjet, double dReph, double dRmuph){
  ...
  for( ; ph_itr != ph_end; ++ph_itr ) {
	...
	//	  << " due to electron at (eta,phi)=(" << (*el_itr)->eta() <<"," << (*el_itr)->phi() <<")"<< std::endl ;
	//std::cout << "PHOTON SHOULD BE REMOVED " << std::endl;
	(*ph_itr)->auxdecor<char>("passOR") = 0 ; //dec_passOR( **ph_itr ) = 0;
      }
    }
  }
