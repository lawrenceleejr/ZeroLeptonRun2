...
bool  MCEventVetoHelper::mc12AlpgenZjets_accept(uint32_t channel, const xAOD::TruthParticleContainer* mcparticles)
{
  if ( channel > 156803 && channel < 156829 ) // Alpgen Z+jets
  {
    ...
    if ( ptmax < 0. ) throw std::logic_error("No Z pt cut defined for Algen Zjets MC12 sample !");
    ...
    for ( xAOD::TruthParticleContainer::const_iterator it = mcparticles->begin();
	  it != mcparticles->end(); ++it ){
       ...
       {
	 if ( zpt > 0. ) throw std::logic_error("More that one Z with status=124 in Algen Z+jet sample !");
	 zpt = (*it)->pt();
       }
    }
    if ( zpt < ptmin || zpt >= ptmax ) return false;
  }
  return true;
}


bool  MCEventVetoHelper::mc12AlpgenYjets_accept(uint32_t channel, const xAOD::TruthParticleContainer* mcparticles)
{
  if ( channel >= 156839 && channel <= 156863 ) // Alpgen gamma+jets
  {
    ...
    if ( ptmax < 0. ) throw std::logic_error("No gamma pt cut defined for Algen gamm+jets MC12 sample !");
    ...
    for ( xAOD::TruthParticleContainer::const_iterator it = mcparticles->begin();
      ...
      {
	if ( gampt > 0. ) throw std::logic_error("More that one gamma with status=124 in Algen gamma+jet sample !");
	gampt = (*it)->pt();
      }
    }
    if ( gampt < ptmin || gampt >= ptmax ) return false;
  }
  return true;
}

void MCEventVetoHelper::mc12AlpgenJimmyW_accept(unsigned int& veto,uint32_t channel, const xAOD::TruthParticleContainer* mcparticles, const xAOD::MissingETContainer* metc)
{
  if ( (channel >= 107680 && channel <= 107685) ||
    ...
    for ( xAOD::TruthParticleContainer::const_iterator it = mcparticles->begin();
	  it != mcparticles->end(); ++it ){
      if ((*it)->status() != 123 && (*it)->status() != 124) continue;
      ...
      if ( ( (*it)->absPdgId() <= 6 || (*it)->absPdgId() == 21 ) && 
	   (*it)->pt() > 85000.)
        ishighptq = true;
      if (ishighptnu && ishighptq) break;
    }
    if (isbaseline && ishighptnu && ishighptq) veto += 4;
    if (!isbaseline && !(ishighptnu && ishighptq)) veto += 4;
  }
	    ...
	    (channel >= 172021 && channel <= 172026) ) { // Alpgen+Jimmy Wtaunu+jets
...
void MCEventVetoHelper::mc12SherpaZnunu_accept(unsigned int& veto,uint32_t channel, const xAOD::TruthParticleContainer* mcparticles)
{
  if ( channel >= 157537 && channel <= 157540 ) { // Sherpa Zvv+jets
    ...
    if ( ptmax < 0. ) throw std::logic_error("No Z pt cut defined for Sherpa MC12 Znunu sample !");
    TLorentzVector tlvZ(0.,0.,0.,0.);
    for ( xAOD::TruthParticleContainer::const_iterator it = mcparticles->begin();
	  it != mcparticles->end(); ++it ){
      if ( (*it)->status() == 3 && ( (*it)->absPdgId() == 12 || 
				     (*it)->absPdgId() == 14 || 
				     (*it)->absPdgId() == 16   ) ) {
        tlvZ += (*it)->p4();
      }
    }
    if ( tlvZ.Pt() < ptmin || tlvZ.Pt() >= ptmax ) veto += 1;
  } 
}

void MCEventVetoHelper::mc12SherpaYjets_accept(unsigned int& veto,uint32_t channel, const xAOD::TruthParticleContainer* mcparticles)
{
  if ( (channel >= 113714 && channel <= 113717) ||
       (channel == 126371) ||
    ...
    if ( ptmax < 0. ) throw std::logic_error("No gamma pt cut defined for Sherpa MC12 sample !");
    ...
    if ( gampt < ptmin || gampt >= ptmax ) veto += 1;
  }
}
...
bool MCEventVetoHelper::mc12SherpaWZjets_accept(unsigned int& veto,
						uint32_t channel, 
						const xAOD::TruthParticleContainer* mcparticles)
{
  if (channel >= 167150 && channel <= 167155) {// Sherpa W+jets MassiveB
    ...
    if (vboson.Pt() > 140000.) veto += 200;
    
  } 

  else if ((channel >= 167740 && channel <= 167748) || (channel >= 167749 && channel <= 167760)) {// Sherpa W/Z+jets MassiveCB
    TLorentzVector vboson;
    bool isbosonfound = MCEventVetoHelper::trueBosonFromWorZplusJetsMCSample(vboson, channel, mcparticles);
    if (!isbosonfound) return true;
  ...
  else if ((channel >= 144992 && channel <= 144994) || (channel >= 147774 && channel <= 147776)) { // Sherpa W+jets
    TLorentzVector vboson;
    bool isbosonfound = MCEventVetoHelper::trueBosonFromWorZplusJetsMCSample(vboson, channel, mcparticles);
    if (!isbosonfound) return true;
    if (vboson.Pt() > 210000.) veto += 100;
    if (channel >= 144992 && channel <= 144994) return true;

    const xAOD::TruthVertex*originVertex = 0;
    for ( xAOD::TruthParticleContainer::const_iterator it = mcparticles->begin();
	  it != mcparticles->end(); ++it ){
      if ( (*it)->absPdgId()>=11 && (*it)->absPdgId()<=16 && (*it)->status()==3) {
	(*it)->prodVtx();
	break;
      }
    }
    if ( originVertex ) {
      if (originVertex->nOutgoingParticles()-2 >= 3) veto += originVertex->nOutgoingParticles()-2;
    }
  }
  return true;
}

void MCEventVetoHelper::mc12HerwigVVjets_accept(unsigned int& veto,
						uint32_t channel, 
						const xAOD::TruthParticleContainer* mcparticles)
{
  if (channel == 105985 || channel == 105987) { // Herwig WW or WZ with 1 el or 1 mu filter => skip events with a W->tau (taken into account by 157951 or 157952)
    bool foundtau = false;
    for ( xAOD::TruthParticleContainer::const_iterator it = mcparticles->begin();
	  it != mcparticles->end(); ++it ){
      if ( (*it)->absPdgId() == 24 && (*it)->status() == 195 ) {
	const xAOD::TruthVertex* decayV = 0;
	if ( (*it)->hasDecayVtx() ) decayV = (*it)->decayVtx();
	size_t nDaughters = decayV->nOutgoingParticles();
        for (size_t ichild = 0; ichild < nDaughters; ichild++) {
          if ( decayV->outgoingParticle(ichild)->absPdgId() != 15) continue;
          foundtau = true;
          veto += 15;
	  break;
        }
  ...
  else if (channel >= 161995 && channel <= 161997) { // Herwig WW/WZ/ZZ without filter : skip events with tau and 1 lepton
    bool foundlep = false;
    for ( xAOD::TruthParticleContainer::const_iterator it = mcparticles->begin();
	  it != mcparticles->end(); ++it ){
    ...
    if (foundtau||foundlep) veto += 1000;
  }
}
...
bool MCEventVetoHelper::mc12accept(unsigned int& veto,uint32_t mc_channel_number, const xAOD::TruthParticleContainer* mcparticles, const xAOD::MissingETContainer* metc)
{
  ...
  if ( !MCEventVetoHelper::mc12AlpgenZjets_accept(mc_channel_number, mcparticles) ) return false;

  if ( !MCEventVetoHelper::mc12AlpgenYjets_accept(mc_channel_number, mcparticles
) ) return false;

  MCEventVetoHelper::mc12AlpgenJimmyW_accept(veto,mc_channel_number, mcparticles, metc);

  MCEventVetoHelper::mc12SherpaZnunu_accept(veto,mc_channel_number, mcparticles);
  MCEventVetoHelper::mc12SherpaYjets_accept(veto,mc_channel_number, mcparticles);

  if ( !MCEventVetoHelper::mc12SherpaWZjets_accept(veto,mc_channel_number, mcparticles) ) return false;

  MCEventVetoHelper::mc12HerwigVVjets_accept(veto,mc_channel_number, mcparticles);
