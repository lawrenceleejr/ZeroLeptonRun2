    ...
    m_treeRepository()
{
  ...
  m_suffixSyst = "test";
  m_suffixSmear = config.get("suffixSmear","");
  m_physobjsFiller = new PhysObjProxyFiller(20000.f,10000.f,10000.f,25000.f,m_suffix,m_doRecl,m_suffixRecl,m_suffixSyst,m_suffixSmear);
...
bool ZeroLeptonSR::processEvent(xAOD::TEvent& event)
{
  ...
  if(! m_IsTruth){
    if ( ! store->retrieve<TVector2>(missingET,"SUSYMET"+m_suffix+m_suffixSmear+systag).isSuccess() ) throw std::runtime_error("could not retrieve SUSYMET"+m_suffix+m_suffixSmear+systag);
  ...
  auto isZnunuMC= [](uint32_t mc_channel_number) {
    return ((mc_channel_number>=361444 && mc_channel_number<=361467) || // Sherpa Znunu NLO
	    (mc_channel_number>=407163 && mc_channel_number<=407183) // Sherpa Znunu LO
	   );
  };

  TLorentzVector znunu;
  if( !m_IsData &&  m_IsTruth && isZnunuMC(mc_channel_number) ) {
    ...
    m_extrantv.ZvvPt  = znunu.Pt()/1000.;
    m_extrantv.ZvvEta = znunu.Eta()/1000.;
    m_extrantv.ZvvPhi = znunu.Phi()/1000.;
    m_extrantv.ZvvM   = znunu.M()/1000.;
  }
