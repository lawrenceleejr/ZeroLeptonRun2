...
void PhysObjProxyUtils::FillNTVars(NTVars& ntv, 
				   ...
				   const std::vector<JetProxy>& good_fat_jets,
				   const std::vector<float>& vD2_fat,
				   const std::vector<bool>& visWmedium_fat,
				   int hardproc,
				   ...
				   std::vector<TauProxy> signal_taus)
{
  ...
  ntv.METPhi = METPhi;
  ntv.METTST = MissingEtTST * 0.001;
  ntv.METTSTPhi = METTSTPhi;
  ...
  */
  ...
  ntv.NFatJet = 0;
  for ( size_t jet0=0; jet0<good_fat_jets.size(); jet0++)
    {
      const JetProxy& thisjet = good_fat_jets[jet0];
      float fat_pt = thisjet.Pt();
      float fat_eta = (fat_pt > 0.) ? thisjet.Eta() : 0.;
      float fat_phi = (fat_pt > 0.) ? thisjet.Phi() : 0.;
      float fat_m = (fat_pt > 0.) ? thisjet.M() : 0.;
      
      if (fat_pt > 50000.) {
	ntv.FatJetPt.push_back(fat_pt * 0.001);
	ntv.FatJetEta.push_back(fat_eta);
	ntv.FatJetPhi.push_back(fat_phi);
	ntv.FatJetM.push_back(fat_m * 0.001);
	ntv.NFatJet++;
      }
    }
  ntv.D2_fat = vD2_fat;
  ntv.isWmedium_fat = visWmedium_fat;
