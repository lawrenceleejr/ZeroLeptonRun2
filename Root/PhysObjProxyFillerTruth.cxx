					     ...
					     std::vector<ElectronTruthProxy>& isolated_signal_electrons)
{
  ...
  if ( !store->retrieve(electrons, "myTruthElectronsIso"+m_suffix).isSuccess() ){
    throw std::runtime_error("Could not retrieve ElectronContainer with key myTruthElectronsIso");
  }
  ...
  for ( xAOD::TruthParticleContainer::const_iterator it = electrons->begin();
    ...
    if ( std::abs((*it)->eta()) < 2.47 ) {
      ...
      if((*it)->auxdata<unsigned int>("classifierParticleType")==2){
					 ...
					 std::vector<MuonTruthProxy>& isolated_signal_muons)
{
  ...
  if ( !store->retrieve(muons, "myTruthMuonsIso"+m_suffix).isSuccess() ){
    throw std::runtime_error("Could not retrieve MuonContainer with key myTruthMuonsIso");
  }
  ...
  for ( xAOD::TruthParticleContainer::const_iterator it = muons->begin();
    ...
    if ( (*it)->auxdecor<char>("passOR") == 0) continue;
    if ( std::abs((*it)->eta()) < 2.7 ) {
      // https://svnweb.cern.ch/trac/atlasoff/browser/PhysicsAnalysis/MCTruthClassifier/trunk/MCTruthClassifier/MCTruthClassifierDefs.h
      if((*it)->auxdata<unsigned int>("classifierParticleType")==6){
						...
						std::vector<PhotonTruthProxy>& isolated_signal_photons)
{
  ...
  if ( !store->retrieve(photons, "myTruthPhotonsIso"+m_suffix).isSuccess() ){
    throw std::runtime_error("Could not retrieve PhotonContainer with key myTruthPhotonsIso");
  }

  for ( xAOD::TruthParticleContainer::const_iterator it = photons->begin();
        it != photons->end(); ++it ){
    if ( (*it)->pt() < m_phPtCut ) continue;
    if ((*it)->auxdecor<char>("passOR") != 1) continue;
    if ( std::abs((*it)->eta()) < 2.37 ) {
      if((*it)->auxdata<unsigned int>("classifierParticleType")==14){
