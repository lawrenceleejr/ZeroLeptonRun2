...
void parse_file (const string &filename, map<string,string> &dict)
  ...
  // if there's an env variable starting the string, expand it
  if ( expandedFileName.find("${")!=std::string::npos ){
    // gets env variable
    int startPosition = expandedFileName.find_first_of("{");
    int endPosition = expandedFileName.find_first_of("}");
    string tmpEnvVar = expandedFileName.substr(startPosition+1, endPosition-2);
    //chops off env var
    expandedFileName = expandedFileName.substr(endPosition+1,expandedFileName.back()+1);
    //adds expanded variable to filename
    expandedFileName = getenv(tmpEnvVar.c_str()) + expandedFileName;
  }

  ifstream inp(expandedFileName.c_str());
